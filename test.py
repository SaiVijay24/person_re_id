...
if 'use_swin' in config:
    opt.use_swin = False#config['use_swin']

if 'use_efficient' in config:
    opt.use_efficient = False#config['use_efficient']
    ...
    for iter, data in enumerate(dataloaders):
        ...
        for i in range(2):
            ...
            for scale in ms:
                ...
                traced_cell = torch.jit.trace(model, (input_img))
                torch.jit.save(traced_cell, "model_jit.pth")
                pdb.set_trace()
                ff += outputs
        # norm feature
        if opt.PCB:
